    ...
    <div class="row">
        ...
        <div class="col-lg-12 col-xs-6">
          ...
          <div class="small-box" style="background:#007a64; color: white">
            <div class="inner">
              <center><h2>Gerenciador de Especificações de Treinamento</h2></center>              
                <h4>
                    <div align="right">
                    <button type="button" class="btn btn-success" data-toggle="modal" data-target="     .bd-example-modal-lg" >Incluir Especificação de Treinamento</button>
                    </div>
                </h4>
            </div>
          </div>
...
<head>
    ...
    <table id="tobarril" class="table table-bordered" border="1" bgcolor="#ffffff">
        ...
        @foreach ($espectreinamentos as $espectreinamento)
        <tbody>
        <tr >
            <td><center>{{ $espectreinamento->id }}</center></td>
            <td><center>{{ $espectreinamento->nome_espectreinamento }}</center></td>
            <td>
                <center>
                <form action="{{ route('espectreinamentos.destroy',$espectreinamento->id) }}" method="POST" onsubmit="return confirm ('Tem certeza que deseja excluir o Tipo de Treinamento selecionado?')">



 
                    <a class="btn btn-primary" href="{{ route('espectreinamentos.edit',$espectreinamento->id) }}">Editar</a>
